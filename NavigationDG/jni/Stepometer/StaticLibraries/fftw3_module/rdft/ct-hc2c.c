...
static void apply_dit(const plan *ego_, float *r0, float *r1, float *cr, float *ci)
{
...
static void apply_dif(const plan *ego_, float *r0, float *r1, float *cr, float *ci)
{
     const P *ego = (const P *) ego_;
     plan_rdft *cld;
     plan_hc2c *cldw;
...
static void apply_dit_dft(const plan *ego_, float *r0, float *r1, float *cr, float *ci)
{
     const P *ego = (const P *) ego_;
     plan_dft *cld;
     plan_hc2c *cldw;
...
static void apply_dif_dft(const plan *ego_, float *r0, float *r1, float *cr, float *ci)
{
     const P *ego = (const P *) ego_;
     plan_dft *cld;
     plan_hc2c *cldw;
...
int fftwf_hc2c_applicable(const hc2c_solver *ego, const problem *p_,
		       planner *plnr)
{
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr)
{
     ...
     if (!fftwf_hc2c_applicable(ego, p_, plnr))
          return (plan *) 0;
		  ...
		  case HC2C_VIA_RDFT:
		       cld = fftwf_mkplan_d(
			    ...
			    fftwf_mkproblem_rdft_1_d(
				 fftwf_mktensor_1d(m, (r/2)*d[0].is, d[0].os),
				 fftwf_mktensor_3d(
	 ...
	 case HC2R:
		  ...
		  case HC2C_VIA_RDFT:
		       cld = fftwf_mkplan_d(
			    plnr,
			    fftwf_mkproblem_rdft_1_d(
				 fftwf_mktensor_1d(m, d[0].is, (r/2)*d[0].os),
				 fftwf_mktensor_3d(
			      ...
			      hc2c_mkinferior mkcldw)
{
     ...
     hc2c_solver *slv = (hc2c_solver *)fftwf_mksolver(size, &sadt);
     slv->r = r;
...
plan *fftwf_mkplan_hc2c(size_t size, const plan_adt *adt, hc2capply apply)
{
     plan_hc2c *ego;

     ego = (plan_hc2c *) fftwf_mkplan(size, adt);
     ego->apply = apply;
