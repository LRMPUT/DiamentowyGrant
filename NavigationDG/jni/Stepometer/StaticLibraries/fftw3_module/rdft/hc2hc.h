...
typedef void (*hc2hcapply) (const plan *ego, float *IO);
...
extern plan *fftwf_mkplan_hc2hc(size_t size, const plan_adt *adt,
			     hc2hcapply apply);

#define MKPLAN_HC2HC(type, adt, apply) \
  (type *)fftwf_mkplan_hc2hc(sizeof(type), adt, apply)
...
extern hc2hc_solver *(*fftwf_mksolver_hc2hc_hook)(size_t, INT, hc2hc_mkinferior);

void fftwf_regsolver_hc2hc_direct(planner *plnr, khc2hc codelet,
			       const hc2hc_desc *desc);

int fftwf_hc2hc_applicable(const hc2hc_solver *, const problem *, planner *);
