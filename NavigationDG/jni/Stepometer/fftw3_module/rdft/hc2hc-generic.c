...
static void mktwiddle(P *ego, enum wakefulness wakefulness)
{
	...
	access both to data and twiddles */
     fftwf_twiddle_awake(wakefulness, &ego->td, tw,
		      ego->r * ego->m, ego->m, ego->r);
}

static void bytwiddle(const P *ego, float *IO, float sign)
{
     INT i, j, k;
     ...
     for (i = 0; i < vl; ++i, IO += vs) {
	  ...
	  for (k = 1, W += (m - 1) + 2*(mstart1-1); k < r; ++k) {
	       ...
	       float *pr = IO + mstart1 * s + k * ms;

	       /* pi := IO + (m - j - mstart1) * s + k * ms */
	       float *pi = IO - mstart1 * s + (k + 1) * ms;
...
static void swapri(float *IO, INT r, INT m, INT s, INT jstart, INT jend)
{
     INT k;
     INT ms = m * s;
     INT js = jstart * s;
     for (k = 0; k + k < r; ++k) {
	  /* pr := IO + (m - j) * s + k * ms */
	  float *pr = IO + (k + 1) * ms - js;
	  /* pi := IO + (m - j) * s + (r - 1 - k) * ms */
	  float *pi = IO + (r - k) * ms - js;
	  INT j;
...
static void reorder_dit(const P *ego, float *IO)
{
     INT i, k;
     INT r = ego->r, m = ego->m, s = ego->s, vl = ego->vl, vs = ego->vs;
     INT ms = m * s;
...
static void reorder_dif(const P *ego, float *IO)
{
     INT i, k;
     INT r = ego->r, m = ego->m, s = ego->s, vl = ego->vl, vs = ego->vs;
     INT ms = m * s;
...
static void apply_dit(const plan *ego_, float *IO)
{
...
static void apply_dif(const plan *ego_, float *IO)
{
     const P *ego = (const P *) ego_;
     INT start;
     plan_rdft *cld, *cld0;
		    ...
		    float *IO, planner *plnr)
{
     ...
     cld0 = fftwf_mkplan_d(plnr,
			fftwf_mkproblem_rdft_1_d(
			     mstart == 0 ? fftwf_mktensor_1d(r, m * s, m * s)
			     : fftwf_mktensor_0d(),
			     fftwf_mktensor_1d(vl, vs, vs),
			     IO, IO, kind)
			);
     if (!cld0) goto nada;
	...
	(where 2 corresponds to the real and imaginary parts) ...
     ...
     cld = fftwf_mkplan_d(plnr,
			fftwf_mkproblem_rdft_1_d(
			     fftwf_mktensor_1d(r, m * s, m * s),
			     fftwf_mktensor_3d(2, mstride * s, mstride * s,
					    mcount1, s, s,
					    vl, vs, vs),
...
{
     S *slv = (S *)fftwf_mksolver_hc2hc(sizeof(S), r, mkcldw);
     REGISTER_SOLVER(plnr, &(slv->super));
     if (fftwf_mksolver_hc2hc_hook) {
	  slv = (S *)fftwf_mksolver_hc2hc_hook(sizeof(S), r, mkcldw);
	  REGISTER_SOLVER(plnr, &(slv->super));
     }
}

void fftwf_hc2hc_generic_register(planner *p)
{
     regsolver(p, 0);
}
