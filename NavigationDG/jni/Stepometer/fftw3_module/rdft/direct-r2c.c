...
static void dobatch_r2hc(const P *ego, float *I, float *O, float *buf, INT batchsz)
{
     fftwf_cpy2d_ci(I, buf,
     ...
     if (IABS(WS(ego->csr, 1)) < IABS(ego->ovs)) {
	  ...
	  ego->k(buf, buf + WS(ego->bcsr /* hack */, 1),
		 ...
		 batchsz, 1, 1);
	  fftwf_cpy2d_co(buf, O,
		      ego->n, WS(ego->bcsr, 1), WS(ego->csr, 1),
		      batchsz, 1, ego->ovs, 1);
     }
}

static void dobatch_hc2r(const P *ego, float *I, float *O, float *buf, INT batchsz)
{
...
static void iterate(const P *ego, float *I, float *O,
		    void (*dobatch)(const P *ego, float *I, float *O,
				    float *buf, INT batchsz))
{
     ...
     STACK_MALLOC(float *, buf, n * batchsz * sizeof(float));
...
static void apply_buf_r2hc(const plan *ego_, float *I, float *O)
{
     iterate((const P *) ego_, I, O, dobatch_r2hc);
}

static void apply_buf_hc2r(const plan *ego_, float *I, float *O)
{
...
static void destroy(plan *ego_)
{
     ...
     fftwf_stride_destroy(ego->csr);
     fftwf_stride_destroy(ego->csi);
     fftwf_stride_destroy(ego->brs);
     fftwf_stride_destroy(ego->bcsr);
     fftwf_stride_destroy(ego->bcsi);
}
...
static void print(const plan *ego_, printer *p)
{
     ...
     if (ego->slv->bufferedp)
	  p->print(p, "(rdft-%s-directbuf/%D-r2c-%D%v \"%s\")",
		   fftwf_rdft_kind_str(s->desc->genus->kind),
		   /* hack */ WS(ego->bcsr, 1), ego->n,
		   ego->vl, s->desc->nam);

     else
	  p->print(p, "(rdft-%s-direct-r2c-%D%v \"%s\")",
		   fftwf_rdft_kind_str(s->desc->genus->kind), ego->n,
		   ego->vl, s->desc->nam);
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr)
{
     ...
     pln->rs0 = rs;
     pln->rs = fftwf_mkstride(n, 2 * rs);
     pln->csr = fftwf_mkstride(n, cs);
     pln->csi = fftwf_mkstride(n, -cs);
     pln->ioffset = ioffset(p->kind[0], n, cs);

     b = compute_batchsize(n);
     pln->brs = fftwf_mkstride(n, 2 * b);
     pln->bcsr = fftwf_mkstride(n, b);
     pln->bcsi = fftwf_mkstride(n, -b);
...
solver *fftwf_mksolver_rdft_r2c_direct(kr2c k, const kr2c_desc *desc)
{
     return mksolver(k, desc, 0);
}

solver *fftwf_mksolver_rdft_r2c_directbuf(kr2c k, const kr2c_desc *desc)
{
