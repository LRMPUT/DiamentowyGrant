...
/* plan.c: */
typedef void (*rdftapply) (const plan *ego, float *I, float *O);
...
#define MKPLAN_RDFT(type, adt, apply) \
  (type *)fftwf_mkplan_rdft(sizeof(type), adt, apply)
...
void fftwf_rdft_generic_register(planner *p);
void fftwf_rdft_rader_hc2hc_register(planner *p);
...
problem *fftwf_mkproblem_rdft2_d_3pointers(tensor *sz, tensor *vecsz,
					float *r, float *cr, float *ci, rdft_kind kind);
...
/* verify.c: */
void fftwf_rdft2_verify(plan *pln, const problem_rdft2 *p, int rounds);

/* solve.c: */
void fftwf_rdft2_solve(const plan *ego_, const problem *p_);

/* plan.c: */
typedef void (*rdft2apply) (const plan *ego, float *r0, float *r1, float *cr, float *ci);
...
#define MKPLAN_RDFT2(type, adt, apply) \
  (type *)fftwf_mkplan_rdft2(sizeof(type), adt, apply)
