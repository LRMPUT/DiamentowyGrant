...
/**************************************************************/
static void bytwiddle(const P *ego, INT mb, INT me, float *buf, float *rio, float *iio)
{
     INT j, k;
...
static void dobatch(const P *ego, INT mb, INT me, float *buf, float *rio, float *iio)
{
     plan_dft *cld;
     INT ms = ego->ms;
...
static void apply(const plan *ego_, float *rio, float *iio)
{
     ...
     float *buf = (float *) MALLOC(sizeof(float) * 2 * BATCHDIST(ego->r) * ego->batchsz,
			   BUFFERS);
     INT m;
...
static void awake(plan *ego_, enum wakefulness wakefulness)
{
     ...
     switch (wakefulness) {
	 case SLEEPY:
	      fftwf_triggen_destroy(ego->t); ego->t = 0;
	      break;
	 default:
	      ego->t =fftwf_mktriggen(AWAKE_SQRTN_TABLE, ego->r * ego->m);
	      break;
     }
}
		    ...
		    planner *plnr)
{
     ...
     buf = (float *) MALLOC(sizeof(float) * 2 * BATCHDIST(r) * ego->batchsz, BUFFERS);
     cld = fftwf_mkplan_d(plnr,
			fftwf_mkproblem_dft_d(
			     fftwf_mktensor_1d(r, 2, 2),
			     fftwf_mktensor_1d(ego->batchsz,
					    2 * BATCHDIST(r),
					    2 * BATCHDIST(r)),
			     buf, buf + 1, buf, buf + 1
			     )
...
{
     S *slv = (S *)fftwf_mksolver_ct(sizeof(S), r, DECDIT, mkcldw, 0);
     slv->batchsz = batchsz;
     REGISTER_SOLVER(plnr, &(slv->super.super));

     if (fftwf_mksolver_ct_hook) {
	  slv = (S *)fftwf_mksolver_ct_hook(sizeof(S), r, DECDIT, mkcldw, 0);
	  slv->batchsz = batchsz;
	  REGISTER_SOLVER(plnr, &(slv->super.super));
     }
